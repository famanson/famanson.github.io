...
To bring our database over to the new infrastructure without any downtime, we used a technique in Couchbase called XDCR (Cross-DataCenter Replication). The idea is building the new cluster, and then set up a flow of data from the old cluster into the new one, effectively replicating it on any update. Once all the old data is in place, one can simply flip the switch for the application to use the new cluster, and all the precious data will be there in the new one. Once the rest of the XDCR ops finish, we can make a backup of the old server and then archive it. 
